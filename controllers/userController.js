...
class UserController {
	  ...
	  let newUser = new User ({
		...
		password: req.body.password 
	      ...
	      newUser.save((err, result)=>{
		      if (err) res.status(500).json({error:"Something wrong, please contact developer!"});
  ...
  static login(req, res){
    User.findOne({ $or:[{username:req.query.identity},{email:req.query.identity}]})
      .then(user => {
	  ...
	  if (err) res.status(500).json({error:"email or password didn't match, please try again!"})
	    ...
	    if (isMatch){
	      ...
	      jwt.sign(data, 'thisisoursecret',(err, token)=>{
	        if(err) {
		  console.log(err)
		  res.status(500).json({error:"Something wrong, please contact developer!"})
		} else {
